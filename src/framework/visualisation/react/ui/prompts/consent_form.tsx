...
// Define the table IDs we want to hide from the UI
const HIDDEN_TABLE_IDS = ['id', 'metadata'];

export const ConsentForm = (props: Props): JSX.Element => {
  // Filter out the tables with IDs matching our hidden IDs list
  const filteredTables = props.tables.filter(table => !HIDDEN_TABLE_IDS.includes(table.id))
  
  const tablesIn = React.useRef<Array<PropsUITable & TableContext>>(parseTables(filteredTables))
  // We still need all meta tables for backend processing
  ...
  function handleTableChange (id: string, rows: PropsUITableRow[]): void {
    console.debug(`[ConsentForm] User modified table ${id}, new row count: ${rows.length}`)
  ...
  function handleDonate (): void {
    ...
    // including the hidden ones, which is why we access props.tables directly
    const allTables = parseTables(props.tables)
    
    // Update our filtered tables first
    const filteredTablesCopy = tablesOut.current.slice(0)
    
    // Create a combined array where we take edited tables from tablesOut
    // and unedited tables from allTables
    const combinedTables = allTables.map(table => {
      const editedTableIndex = filteredTablesCopy.findIndex(t => t.id === table.id)
      return editedTableIndex >= 0 ? filteredTablesCopy[editedTableIndex] : table
    })
    
    // Use the combinedTables for serialization
  ...
  function handleCancel (): void {
    console.debug('[ConsentForm] User clicked Cancel button')
  ...
  function serializeTablesArray (tables: Array<PropsUITable & TableContext>): any[] {
